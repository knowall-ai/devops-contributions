...
import { CommitContribution, IContributionProvider, ContributionName } from "../contracts";
...
function getCommits(
  ...
  const commitsUrl =
    ...
    "&fromDate=" +
    encodeURIComponent(fromDate.toJSON()) +
    "&author=" +
  ...
  callApi(
    ...
    undefined,
    undefined,
    (commits) => defered.resolve(commits.value),
...
async function commitsForRepository(
  username: string,
  repoId: string,
  skip = 0
): Promise<GitCommitRef[]> {
  // Cast Q.IPromise to native Promise for TypeScript 5 compatibility
  const commitsPromise = getCommits(repoId, yearStart, skip, batchSize, username) as unknown as Promise<GitCommitRef[]>;
  return commitsPromise.then((commits) => {
    if (commits.length < batchSize) {
      return commits.filter((c) => !c.comment.match(/Merged PR \d+/));
    } else {
      return commitsForRepository(username, repoId, skip + batchSize).then((moreCommits) => [
        ...commits,
        ...moreCommits,
      ]);
    }
