...
const batchSize = 101;
function getPullRequestsForRepository(
  repoId: string,
  ...
  return Promise.all([
    ...
    return getPullRequestsForRepository(repoId, searchCriteria, skip + batchSize).then(
      (morePullrequests) => [...pullrequests, ...morePullrequests]
    );
  });
}
...
export async function getCreatedPullRequests(
  filter: IIndividualContributionFilter
): Promise<GitPullRequest[]> {
...
export async function getReviewedPullRequests(
  filter: IIndividualContributionFilter
): Promise<GitPullRequest[]> {
  if (!filter.identity.id) {
    return []; // requires userId to be able to do lookup
  ...
  for (const { key: repoId } of filter.repositories) {
    if (!(repoId in reviewedPrs[filter.identity.id])) {
      reviewedPrs[filter.identity.id][repoId] = getPullRequestsForRepository(
        repoId,
        searchCriteria
      );
    }
...
export class ClosePullRequestProvider implements IContributionProvider {
  ...
  public getContributions(filter: IIndividualContributionFilter): Promise<UserContribution[]> {
    return getCreatedPullRequests(filter).then((pullrequests) =>
      pullrequests.filter((pr) => pr.closedDate).map((pr) => new ClosePullRequestContribution(pr))
    );
  }
}
...
export class ReviewPullRequestProvider implements IContributionProvider {
  ...
  public getContributions(filter: IIndividualContributionFilter): Promise<UserContribution[]> {
    return getReviewedPullRequests(filter).then((pullrequests) =>
      pullrequests.filter((pr) => pr.closedDate).map((pr) => new ReviewPullRequestContribution(pr))
    );
  }
}
