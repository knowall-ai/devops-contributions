...
async function hardGetAllIdentitiesInTeam(
  project: { id: string; name: string },
  team: WebApiTeam
): Promise<ITeamIdentities> {
  const teamIdentity = <IdentityRef>{
...
function hardGetAllIdentitiesInProject(project: {
  id: string;
  name: string;
}): IPromise<IProjectIdentities> {
  async function hardGetAllIdentitiesInProjectImpl(
    project: { id: string; name: string },
    skip: number
  ): Promise<IProjectIdentities> {
    ...
    return Promise.all([teamPromises, moreTeams]).then(
      ([teams, moreTeams]): IProjectIdentities => ({
        id: project.id,
        name: project.name,
...
function hardGetAllIdentitiesInAllProjects(): IPromise<IProjectIdentities[]> {
  return getClient()
    .getProjects()
    .then((projects) => Promise.all(projects.map((p) => hardGetAllIdentitiesInProject(p))));
}
