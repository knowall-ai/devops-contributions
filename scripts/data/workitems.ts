...
function getWorkItems(ids: number[]): Promise<WorkItem[]> {
  ...
  while (coldIds.length > 0) {
    ...
    const wisPromise = getClient()
      .getWorkItems(idBatch)
      .then((wis) => {
...
function getWorkItemsForQuery(query: string): Promise<WorkItem[]> {
  ...
  async function getResults(earlierThanDate: string = new Date().toJSON()): Promise<WorkItem[]> {
    ...
    const fullQuery = query.replace("#earlierThanDate", earlierThanDate);
    const results = await getClient().queryByWiql(
      { query: fullQuery },
      project,
      undefined,
      true,
    ...
    return getResults(newDate).then((moreWorkItems) => [...workitems, ...moreWorkItems]);
  }
...
export class CreateWorkItemContributionProvider implements IContributionProvider {
  ...
  public getContributions(filter: IIndividualContributionFilter): Promise<UserContribution[]> {
    ...
    if (!(query in this.queryResults)) {
      this.queryResults[query] = new CachedValue(() =>
        getWorkItemsForQuery(query).then((wis) =>
