...
import { ChangesetContribution, IContributionProvider, ContributionName } from "../contracts";
...
async function getChangeSets(
  ...
  for (let i = 0; i < batchCount; i++) {
    promises.push(
      getClient().getChangesets(project, undefined, skip + batchSize * i, batchSize, undefined, {
        author: username,
...
export class ChangsetContributionProvider implements IContributionProvider {
  ...
  public async getContributions({
    identity,
    allProjects,
  }: IIndividualContributionFilter): Promise<ChangesetContribution[]> {
    ...
    const changesetsArr = await Promise.all(
      projects.map(async (p) =>
        changesetsCache[username][p].then((changesets) =>
          changesets.map((c) => new ChangesetContribution(c, p))
        )
      )
    );
