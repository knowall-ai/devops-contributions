...
function getContributionClassDelegate(
  contributions: IUserContributions,
  overrideLargest: number
): (count: number) => string {
  const counts: number[] = Object.keys(contributions.data).map(
    (day) => contributions.data[day].length
  );
  if (counts.length === 0) {
    return () => "";
  }
...
> {
  ...
  render() {
    const getWorkClass = getContributionClassDelegate(
      this.props.contributions,
      this.props.overrideLargest
    );
    const date = new Date();
    ...
    return (
      <div className="user-contributions">
        ...
        />
        <div className={`graph`}>
          <div className="month-labels">{this.getMonths(monthIdxes)}</div>
          <FocusZone className="year" direction={FocusZoneDirection.bidirectional}>
            {this.getDays(weeks)}
            {this.props.loading ? (
              <Spinner className="graph-spinner" size={SpinnerSize.large} />
            ) : null}
          </FocusZone>
        </div>
        <TimeWindow
